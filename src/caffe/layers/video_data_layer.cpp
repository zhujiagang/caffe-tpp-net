...
void VideoDataLayer<Dtype>:: DataLayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
	...
	if(average_duration_2 >= new_length){
		for (int i = 0; i < num_segments; ++i){
			...
			offsets.push_back(int(offset + i*average_duration_2));
		}
	}else {
	...
	//	LOG(INFO) <<" average_duration: "<<average_duration<< " ii: " << ii << " offsets: " << offsets[ii];
	//}
...
void VideoDataLayer<Dtype>::InternalThreadEntry(){
	...
	for (int item_id = 0; item_id < batch_size; ++item_id){
		...
		//LOG(INFO) <<" average_duration: "<<average_duration;
		//LOG(INFO) <<" average_duration_2: "<<average_duration_2;
		...
		//for(int ii=0; ii < num_segments;++ii){
		//	LOG(INFO) <<"length: "<< (int) lines_duration_[lines_id_] << " average_duration: "<<average_duration<< " ii: " << ii << " offsets: " << offsets[ii];
		//}
